...
y_train=w.loc[:,5]
# 
Adult_train = pd.read_csv("DM_HW2/adult.test.csv", encoding='unicode_escape')
scaler.fit(Adult_train)
scaled_features = scaler.transform(Adult_train)
...
y_test=ww.loc[:,5]
# 
Train=RandomForestRegressor(n_estimators=100, criterion = 'mse',random_state=0)
